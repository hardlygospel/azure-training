---
title: Home
nav_order: 1
permalink: /
---

# Azure Training Hub

[![AZ-900](https://img.shields.io/badge/Exam-AZ--900-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)](./azure-fundamentals/)
[![AZ-801](https://img.shields.io/badge/Exam-AZ--801-0078D4?style=flat-square&logo=microsoftazure&logoColor=white)](./azure-801/)
[![Free](https://img.shields.io/badge/Cost-Free-22c55e?style=flat-square)](https://github.com/hardlygospel/azure-training)

Quick reference guides for passing Azure certifications. Succinct, exam-focused, with Q&A and troubleshooting.

---

## Where to Start

| I want to… | Go here |
|---|---|
| Learn cloud basics & pass AZ-900 | [Azure Fundamentals →](./azure-fundamentals/) |
| Learn admin skills & pass AZ-801 | [Azure Administrator →](./azure-801/) |
| Find an answer fast | [Common Q&A →](./common-qa/) |
| Cram before the exam | [Quick Reference Cheat Sheet →](./QUICK-REFERENCE) |

---

## What's Covered

### Azure Fundamentals (AZ-900) — Beginner
Cloud concepts · Deployment models · Core Azure services · Pricing & SLAs · Security & compliance

### Azure Administrator (AZ-801) — Intermediate
Identity & access · Virtual networking · Compute & VMs · Storage & backup · Monitoring & policy

---

## Study Tips

- **Bold text** = likely exam question
- Read each chapter, then review the Q&A at the end
- Tables and bullet lists are how the exam thinks — read them carefully
- Passing is ~700/1000. You don't need to be perfect.

---

## Links

- [Microsoft Learn](https://learn.microsoft.com) — Free official training
- [Azure Pricing Calculator](https://azure.microsoft.com/pricing/calculator/)
- [Azure Status](https://status.azure.com)
- [Practice Assessments](https://learn.microsoft.com/en-us/certifications/practice-assessments-for-microsoft-certifications)
